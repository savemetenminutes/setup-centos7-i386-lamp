...
# Configure the root user prompt
echo 'Creating the git prompt bash source file in the root home directory...';
cat > /root/.git-prompt.sh << 'EOL'
...
EOL
echo 'Creating the git completion bash source file in the root home directory...';
cat > /root/.git-completion.sh << 'EOL'
...
EOL
echo 'Modifying the root user'"'"'s bash prompt to display the username, host, current directory and git branch/status...';
cat > /root/.bashrc << 'EOL'
# .bashrc
...
PS1='\u@\[`[ $? = 0 ] && X=2 || X=1; tput setaf $X`\]\h\[`tput sgr0`\](VagrantBox|savemetenminutes/centos7-i386-lamp|'$(uname -r)'):$PWD$(__git_ps1 " (%s)")\n\$ '
EOL
echo 'Done. Enter the command ". ~/.bashrc" (without the double quotes) to reload the prompt.';
